...
export default {
  ...
  methods: {
    ...
    addSaleAttrValue(row) {
      // 当你点击销售属性值中的添加按钮的时候需要展示由button变为input，通过当前销售属性的inputVisible控制
      // 挂载在销售属性上的响应式数据控制button与input切换
    ...
    handleInputConfirm(row) {
      ...
      // 新增的销售属性值
      let newSaleAttrValue = { baseSaleAttrId,saleAttr inputValue };
      // 修改字段inputVisible值为false
      row.inputVisible = false;
      this.spu.spuSaleAttrValueList.push(newSaleAttrValue);
    },
    // 保存按钮的回调
    async addOrUpdateSpu() {
      // 整理参数 需要整理照片墙的数据
      //对于图片 携带的参数为imgName,imgUrl
      ...
      if (result.code == 200) {
        ...
        // 通知父亲回到场景0
        this.$emit("change", { scene: 0, flag: this.id ? "修改" : "添加" });
      }
      // 去除数据
    ...
    // 取消按钮的回调
    cancel() {
      // 取消按钮的回调，通知父亲切换场景为0，去除未保存的数据残留
      this.$emit("change", { scence: 0, flag: "" });
      //清理数据 es6中新增的方法可以合并对象
      // 组件实列this._data ，可以操作data中的响应式数据
      // this.$options当前组件的配置对象，配置对象的data函数执行返回的响应式数据为空
      Object.assign(this._data, this.$options.data());
    },
  },
  computed: {
